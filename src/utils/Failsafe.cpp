...
void Failsafe<T>::initGeneral(const T &params, const double &safetyFactor, const int &failsafe_max_way_horizon_size) {
  *this = Failsafe<T>(params);
  ...
  this->min_num_midpts_color_correct = 0;  // Enable color-blind
...
template<typename T>
void Failsafe<T>::initColor(const T &params, const int &failsafe_max_way_horizon_size) {
  *this = Failsafe<T>(params);
  this->max_way_horizon_size = failsafe_max_way_horizon_size;
  this->min_num_midpts_color_correct = 0;  // Enable color-blind
}
template void Failsafe<Params::WayComputer::Search>::initColor(const Params::WayComputer::Search &, const int &);